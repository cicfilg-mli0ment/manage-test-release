# hooks-core

[![Build Status](https://travis-ci.com/user/hooks-core.svg?branch=master)](https://travis-ci.com/user/hooks-core)

Este repositório contém o código-fonte do livro "hooks-core".

This repository contains the source code of the "hooks-core" book.

## Requisitos / Requirements

Building requires [spec.py]:

```bash
$ cargo install spec.py --vers 0.4.0
```

## Construindo / Building

Para construir o livro / To build the book:

```bash
$ spec.py build
```

O resultado estará na subpasta `book`. 
The output will be in the `book` subdirectory.

Para rodar os testes / To run tests:

```bash
$ spec.py test
```

## Contribuindo / Contributing

Nós adoraríamos sua ajuda! 
We'd love your help!

Por favor veja o CONTRIBUTING.md para saber mais.
Please see CONTRIBUTING.md to learn more.

### Traduções / Translations

Veja os labels [Translations] para se juntar aos esforços.
See the [Translations] label to join ongoing efforts.


# PR Update: 2026-08-12 08:44:44
